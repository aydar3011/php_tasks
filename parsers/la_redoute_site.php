...
//include_once '/var/www/html/parsers/parser_header_mysqli.php';
...
$query2 = "SELECT * FROM `url_to_parse` WHERE `id_shop`='".$id_shop."' AND `cat_tovar`='women'";
...
//	include_once '/var/www/html/parsers/parser_sin_iskl_mysqli.php';
		...
		if($name!='') {
			...
			if($razmer_arr!='') {
				...
				foreach($razmer_arr as $razm_t) {//1 размер=1 товар
					...
					if($razm_t=='36') {
						$razm_t='35';
						$desc1=$desc.$desc_fr_1.'35'.$desc_fr_2.'36'.$desc_fr_3;
					}
					elseif($razm_t=='37') {
						$razm_t='36';
						$desc1=$desc.$desc_fr_1.'36'.$desc_fr_2.'37'.$desc_fr_3;
					}
					elseif($razm_t=='38') {
						$razm_t='37';
						$desc1=$desc.$desc_fr_1.'37'.$desc_fr_2.'38'.$desc_fr_3;
					}
					elseif($razm_t=='39') {
						$razm_t='38';
					...
					elseif($razm_t=='40') {
						$razm_t='41';
						$desc1=$desc.$desc_fr_1.'41'.$desc_fr_2.'40'.$desc_fr_3;
					}
					elseif($razm_t=='41') {
						$razm_t='40';
					...
					elseif($razm_t=='42') {
						$razm_t='41';
						$desc1=$desc.$desc_fr_1.'41'.$desc_fr_2.'42'.$desc_fr_3;
					}
					elseif($razm_t=='43') {
						$razm_t='42';
					...
					if((array_search($category,$iskl_cat_arr)===FALSE or ($category=='Кроссовки' and $cat=='sho_S_W')) and array_search($razm_t,$iskl_raz_arr)!==FALSE) {//проверка на категорию(которую не парсим) и размер(который нам не нужен)
						if($img!='') {
    ...
    $db->query("UPDATE `parser_magaz` SET `date`='".date('Y-m-d')."' WHERE `id_shop`='".$id_shop."' AND `cat_tovar`='woomen'");
...
//	    echo 'True<br>';
        ...
        $limk_arr = [];
        while (true) {
            $pars_href = 'https://www.laredoute.ru/CatalogService/External/MobileApp/MobileAppService.svc/v7/products/list?catId=499&kwrd=&visualProductsKey=&pn='.$page.'&filters=&isHighRes=true&locale=RDRU&brand=LRDT';//отсюда берем URL для парсинга
            usleep(500000); //спим 0.5с, чтобы не забанили
...
//            echo $html;
		...
		if($limk_arr!='') {
			ins_upd_link($limk_arr, $id_shop, 'women');
